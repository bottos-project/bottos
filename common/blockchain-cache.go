...
func (self *BlockChainCache) GetBlock(hash Hash) *types.Block {
...
func (self *BlockChainCache) GetBlockByNum(number uint32) *types.Block {
	if hash, ok := self.cache.Get(number); ok {
		return self.GetBlock(hash.(Hash))
	} else {
		return nil
	}
}

func (self *BlockChainCache) HasBlock(hash Hash) bool {
...
func (self *BlockChainCache) Insert(block *types.Block) (*types.Block, error) {
	if self.headBlock == nil {
		...
		if block.PrevBlockHash() == self.headBlock.Hash() {
			self.add(block)
