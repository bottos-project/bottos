...
func (mdb *MongoDBPlugin) insertTxInfoRole(block *types.Block, oids []bson.ObjectId) error {
	...
	for i, trx := range block.Transactions {
		...
		decodedParam, err := contract.ParseParam(mdb.Role, trx.Contract, trx.Method, trx.Param)
		if err != nil {
			continue
		}
...
func (mdb *MongoDBPlugin) SetBalanceOp(accountName string, balance uint64) error {
...
func (mdb *MongoDBPlugin) insertAccountInfoRole(block *types.Block, trx *types.Transaction, oid bson.ObjectId) error {
	...
	if trx.Method == "transfer" {
		data, err := contract.ParseParam(mdb.Role, trx.Contract, trx.Method, trx.Param)
		if err != nil {
